...
- 📊 **Interface Web (Streamlit):** Um painel limpo e organizado por abas (Trabalho, Pessoal, Promoções, Newsletters, Finanças) para ler seus resumos rapidamente.
- 🕒 **Filtro de Tempo:** Processa apenas os e-mails das últimas 6 horas, economizando processamento.
- ⏱️ **Execução em Segundo Plano:** Um script gerenciador (`rodar_tudo.py`) mantém um agendador oculto que processa e-mails automaticamente em horários definidos (ex: 12h e 18h).
...
- **Protocolo de E-mail:** IMAP nativo (`imaplib`)
...
2. Na raiz do projeto, crie um arquivo chamado **`.env`** com as suas credenciais, podendo ser do provedor qualquer.
